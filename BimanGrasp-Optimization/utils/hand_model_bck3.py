...
def extract_mesh_info_from_mjcf(mjcf_path):
    load_mode = "xml_string"  # 'xml_string': to be consistent with the inner functions of pk build_chain_from_mjcf().
...
class HandModel:
    def __init__(
        ...
        if not mjcf_path.endswith(".xml"):
            raise Exception("File format not supported, please use .xml file")
        ...
        model, mesh_dict = extract_mesh_info_from_mjcf(mjcf_path)
        ...
        def build_mesh_recurse(body):
            if len(body.link.visuals) > 0:
                ...
                for visual in body.link.visuals:
                    self.logger.debug(
                        f"link_name: {link_name}, visual.geom_type: {visual.geom_type}, visual.geom_param: {visual.geom_param}"
                    )
                    # geom_type is a np array? e.g. [6], [7]
                    # geom_param is a np array instead of a torch tensor?
                    scale = torch.tensor([1, 1, 1], dtype=torch.float, device=device)

                    if visual.geom_type == mujoco.mjtGeom.mjGEOM_BOX:
                        geom_type = visual.geom_type[0]
                        link_mesh = tm.load_mesh(os.path.join(mesh_path, "box.obj"), process=False)
                        link_mesh.vertices *= visual.geom_param
                    ...
                    elif visual.geom_type[0] == GeomType.MESH:
                        geom_type = visual.geom_type[0]
                        if "shadow" in mjcf_path:
                            link_mesh = tm.load_mesh(
                                os.path.join(mesh_path, visual.geom_param[0].split(":")[1] + ".obj"), process=False
                            )
                        elif "allegro" in mjcf_path:
                            link_mesh = tm.load_mesh(
                                os.path.join(mesh_path, os.path.basename(visual.geom_param)), process=False
                            )
                        if (not isinstance(visual.geom_param, str)) and visual.geom_param[1] is not None:
                            scale = torch.tensor(visual.geom_param[1], dtype=torch.float, device=device)
                    elif visual.geom_type is not None:
                        raise NameError("Unrecogized geom type!")
                ...
                if link_name not in contact_points:  # added by mingrui
                    contact_points[link_name] = []
                contact_candidates = (
                    torch.tensor(contact_points[link_name], dtype=torch.float32, device=device).reshape(-1, 3)
                    if contact_points is not None
                    else None
                )
                penetration_keypoints = (
                    torch.tensor(penetration_points[link_name], dtype=torch.float32, device=device).reshape(-1, 3)
    ...
    def calculate_distance(self, x):
        ...
        for link_name in self.mesh:
            # for shadow hand, skip these links without collision meshes
            if link_name in [
                "robot0:forearm",
                "robot0:wrist_child",
            ...
            if "geom_param" not in self.mesh[link_name] or isinstance(self.mesh[link_name]["geom_param"], str):
                # if not primitive capsule or box, use mesh sdf
            ...
            elif self.mesh[link_name]["geom_param"].size == 2:  # capsule
            ...
            elif self.mesh[link_name]["geom_param"].size == 3:  # box
                box_size = self.mesh[link_name]["geom_param"]
                q = torch.abs(x_local) - box_size.unsqueeze(0)
