...
class MALAOptimizer:
    def __init__(
        ...
        if config is not None:
            ...
            initial_temperature = (
                config.initial_temperature if hasattr(config, "initial_temperature") else config.starting_temperature
            )
            cooling_schedule = (
                config.cooling_schedule if hasattr(config, "cooling_schedule") else config.temperature_decay
            )
            annealing_period = config.annealing_period
            step_size = config.step_size
            stepsize_period = config.stepsize_period
            preconditioning_decay = (
                config.preconditioning_decay if hasattr(config, "preconditioning_decay") else config.momentum
            )
            langevin_noise_factor = config.langevin_noise_factor if hasattr(config, "langevin_noise_factor") else 0.1
            self.joint_limit_clamp: bool = config.joint_limit_clamp
        else:
    ...
    def langevin_proposal(self):
        ...
        current_step_size = self.step_size * self.cooling_schedule ** torch.div(
    ...
    def _langevin_update(self, hand_model, ema_grad, step_size, temperature):
        ...
        if hand_model.hand_pose.grad is not None:
            ...
            adaptive_noise_scale = (
                torch.sqrt(2.0 * step_size_tensor * temperature / (torch.sqrt(ema_grad.value) + 1e-6))
        ...
        adaptive_switch_prob = self.base_switch_possibility * torch.sqrt(temperature / self.initial_temperature).item()
    ...
    def metropolis_hastings_step(self, energy, new_energy):
        ...
        if reject_mask.any():
            with torch.no_grad():
                self.bimanual_pair.restore_states(self.saved_states["left"], self.saved_states["right"], reject_mask)
