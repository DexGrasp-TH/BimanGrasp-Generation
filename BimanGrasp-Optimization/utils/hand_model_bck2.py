...
class HandModel:
    def __init__(
        ...
        if file_path.endswith(".xml"):
            self.build_from_mjcf(file_path, mesh_path, contact_points_path, penetration_points_path, n_surface_points)
        elif file_path.endswith(".urdf"):
            self.build_from_urdf(file_path, mesh_path, contact_points_path, penetration_points_path, n_surface_points)
    ...
    def build_from_mjcf(
        ...
        # load articulation
        self.chain = pk.build_chain_from_mjcf(open(file_path).read()).to(dtype=torch.float, device=device)
        ...
        def build_mesh_recurse(body):
            if len(body.link.visuals) > 0:
                ...
                for visual in body.link.visuals:
                    ...
                    elif visual.geom_type == "mesh" or (
                        visual.geom_type is None and visual.geom_param[0] is not None
                    ):  # added by mingrui
