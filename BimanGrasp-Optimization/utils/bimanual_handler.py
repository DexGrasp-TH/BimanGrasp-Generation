...
class HandState:
    ...
    def clone(self) -> "HandState":
        ...
        return HandState(
            ...
            contact_point_indices=self.contact_point_indices.clone()
...
class BimanualPair:
    ...
    def restore_states(self, left_state: HandState, right_state: HandState, reject_mask: torch.Tensor) -> None:
        """
...
def hand_pose_to_dict(hand_pose: torch.Tensor, joint_names: List[str]) -> Dict[str, float]:
    """
...
def create_grasp_data(
    ...
    # read joint names from the xml
    l_joint_names = left_hand_model.get_joint_names()
    r_joint_names = right_hand_model.get_joint_names()

    # Convert poses to dictionaries
    qpos_left = hand_pose_to_dict(left_hand_model.hand_pose[idx], l_joint_names)
    qpos_left_st = hand_pose_to_dict(left_hand_pose_st[idx], l_joint_names)
