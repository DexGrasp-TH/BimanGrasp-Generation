...
def initialize_convex_hull(left_hand_model, object_model, args):
    ...
    for i in range(n_objects):
        ...
        for j in range(batch_per_obj):
            ...
            world_rot[j] = torch.tensor(
                transforms3d.euler.euler2mat(
                    math.atan2(n[j, 1], n[j, 0]) - np.pi / 2, -math.acos(n[j, 2]), 0, axes="rzxz"
                ),
                dtype=torch.float,
                device=device,
            )
        ...
        translation[start_idx:end_idx] = p - distance.unsqueeze(1) * (world_rot @ z_vec).squeeze(2)
        ...
        rotation[start_idx:end_idx] = world_rot
    ...
    if not args.keep_thumb_contact:
        ...
        # non-thumb fingers
        candidates = left_hand_model.non_thumb_contact_candi_indices
        contact_indices = candidates[torch.randint(len(candidates), (total_batch_size, n_contact), device=device)]
        # thumb
        candidates = left_hand_model.thumb_contact_candi_indices
...
def initialize_dual_hand(right_hand_model, left_hand_model, object_model, args):
    ...
    for i in range(n_objects):
        ...
        z_vec = torch.tensor([0, 0, 1], dtype=torch.float, device=device).reshape(1, -1, 1)
        translation_right[start_idx:end_idx] = p - distance.unsqueeze(1) * (world_rot @ z_vec).squeeze(2)
        # hand_rot = torch.tensor(
        #     transforms3d.euler.euler2mat(0, -np.pi / 3, 0, axes="rzxz"), dtype=torch.float, device=device
        # )
        # rotation_right[start_idx:end_idx] = world_rot @ cone_rot @ hand_rot
        rotation_right[start_idx:end_idx] = world_rot
    ...
    if not args.keep_thumb_contact:
        ...
        # non-thumb fingers
        candidates = right_hand_model.non_thumb_contact_candi_indices
        contact_indices = candidates[torch.randint(len(candidates), (total_batch_size, n_contact), device=device)]
        # thumb
        candidates = right_hand_model.thumb_contact_candi_indices
        contact_indices[:, -1] = candidates[torch.randint(len(candidates), (total_batch_size,), device=device)]
