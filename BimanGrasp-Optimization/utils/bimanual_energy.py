...
class GraspMatrixComputer:
    ...
    def build_grasp_matrix(
        self, contact_points: torch.Tensor, contact_normals: torch.Tensor, batch_size: int, n_contacts: int
    ) -> torch.Tensor:
        """
    ...
    def compute_fc_and_vew(self, G: torch.Tensor) -> tuple:
        ...
        try:
            ...
            log_volume = torch.sum(stable_log(S[:, : min(6, S.shape[1])]), dim=1)
...
class ForceClosureComputer:
    ...
    def compute(self, bimanual_pair: BimanualPair, object_model) -> torch.Tensor:
        ...
        """
        if bimanual_pair.left.contact_points is None or bimanual_pair.right.contact_points is None:
        ...
        all_contact_points = torch.cat([bimanual_pair.left.contact_points, bimanual_pair.right.contact_points], dim=1)
        all_contact_normals = torch.cat([contact_normal_left, contact_normal_right], dim=1)

        # Normalize contact normals
        all_contact_normals = all_contact_normals / (torch.norm(all_contact_normals, dim=-1, keepdim=True) + 1e-8)
...
class PenetrationComputer:
    ...
    def compute_object_penetration(self, bimanual_pair: BimanualPair, object_model) -> torch.Tensor:
        if self.pen_points == "object":
            return self.compute_ho_penetration_o(bimanual_pair, object_model)
        elif self.pen_points == "hand":
            return self.compute_ho_penetration_h(bimanual_pair, object_model)
        else:
            return ValueError(f"Unsupported pen_points type: {self.pen_points}.")

    def compute_ho_penetration_o(self, bimanual_pair: BimanualPair, object_model) -> torch.Tensor:
        """
    ...
    def compute_ho_penetration_h(self, bimanual_pair: BimanualPair, object_model) -> torch.Tensor:
        """
...
class BimanualEnergyComputer:
    ...
    def __init__(self, config: EnergyConfig = None, device="cuda"):
        ...
        self.penetration_computer = PenetrationComputer(config, device)
        self.contact_distance_computer = ContactDistanceComputer(device)
        self.wrench_volume_computer = WrenchVolumeComputer(device)
        self.grasp_matrix_computer = GraspMatrixComputer(device)

    def compute_all_energies(self, bimanual_pair: BimanualPair, object_model, verbose: bool = False) -> EnergyTerms:
        """
        ...
        if bimanual_pair.left.contact_points is not None and bimanual_pair.right.contact_points is not None:
            ...
            all_contact_points = torch.cat(
                [bimanual_pair.left.contact_points, bimanual_pair.right.contact_points], dim=1
            )
            all_contact_normals = torch.cat([contact_normal_left, contact_normal_right], dim=1)
            all_contact_normals = all_contact_normals / (torch.norm(all_contact_normals, dim=-1, keepdim=True) + 1e-8)
