...
def sample_cone_uniform(n_samples: int, axis: torch.Tensor, angle: float, device: torch.device = None) -> torch.Tensor:
    ...
    if torch.allclose(axis, z_axis):
        ...
        vx = torch.tensor([[0, -v[2], v[1]], [v[2], 0, -v[0]], [-v[1], v[0], 0]], device=device, dtype=axis.dtype)
...
def soft_clamp(x: torch.Tensor, min_val: float = None, max_val: float = None, smoothness: float = 0.1) -> torch.Tensor:
    """
...
class Logger:
    ...
    def log(
        ...
        if show:
            print(
                "Step %d energy: %f  fc: %f  dis: %f  pen: %f  spen: %f  joints: %f"
                % (step, energy.mean(), E_fc.mean(), E_dis.mean(), E_pen.mean(), E_spen.mean(), E_joints.mean())
            )
            print(
                "success: %f  fc: %f  dis: %f  pen: %f"
                % (
                    success.float().mean(),
