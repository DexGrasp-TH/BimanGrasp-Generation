...
class GraspExperiment:
    ...
    def run_full_experiment(self):
        ...
        for i in range(len(left_hand_model.joints_names)):
            print(
                f"joint name: {left_hand_model.joints_names[i]}, lb: {left_hand_model.joints_lower[i]}, ub: {left_hand_model.joints_upper[i]},",
                f"init_mu: {self.config.initialization.left_hand_joint_mu[i]}",
            )

        # --- Visualization ---
        # Final poses (solid colors)
        for idx in range(0, 4):
            right_hand_model.set_parameters(self.right_hand_pose_st)
            right_plot = right_hand_model.get_plotly_data(
                i=idx, opacity=1.0, color="lightslategray", with_contact_points=False
            )
            left_hand_model.set_parameters(self.left_hand_pose_st)
            left_plot = left_hand_model.get_plotly_data(
                i=idx, opacity=1.0, color="lightslategray", with_contact_points=False
            )

            object_plot = object_model.get_plotly_data(i=idx, color="seashell", opacity=1.0)

            # Combine everything
            fig = go.Figure(right_plot + left_plot + object_plot)
            fig.update_layout(
